...
export async function getProducts(params: ProductQueryParams = {}): Promise<PaginatedProducts> {
  ...
  if (params.sortBy === "price_asc") orderBy = { price: "asc" };
    ...
    const formattedProducts: MockProduct[] = dbProducts.map((p: any) => ({
      ...
      currency: "NGN",
      stock_quantity: p.variants?.reduce((acc: number, v: any) => acc + (v.inventories?.reduce((s: number, i: any) => s + i.quantity, 0) || 0), 0) || 0,
      ...
      variants: p.variants.map((v: any) => ({
        ...
        stock_quantity: v.inventories?.reduce((s: number, i: any) => s + i.quantity, 0) || 0,
      ...
      rating: p.reviews.length > 0 ? p.reviews.reduce((a: number, b: any) => a + b.rating, 0) / p.reviews.length : 0,
    ...
    return {
      ...
      page,
      limit,
      totalPages: Math.ceil(totalDb / limit) || 1,
    };
}
...
export async function getProductBySlug(slug: string): Promise<MockProduct | null> {
  ...
  if (dbProduct) {
    return {
      ...
      currency: "NGN",
      stock_quantity: dbProduct.variants?.reduce((acc: number, v: any) => acc + (v.inventories?.reduce((s: number, i: any) => s + i.quantity, 0) || 0), 0) || 0,
      ...
      rating: dbProduct.reviews.length > 0 ? dbProduct.reviews.reduce((a: number, b: any) => a + b.rating, 0) / dbProduct.reviews.length : 0,
